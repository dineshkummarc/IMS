				...
				<div class="main-content" >
					<div class="wrap-content container" id="container">
						...
						<div class="breadcrumb-wrapper">
							<h4 class="mainTitle no-margin"><i class="fa fa-edit"></i> Edit Result</h4>
						...
						<h5 class="over-title"><i class="fa fa-edit"></i> Edit Result & Upload Student Documents</h5>
						<form action="{!! Asset('center/result/edit/'.$res->id) !!}" method="post" class="form-login" enctype="multipart/form-data">
						...
						<div class="form-group">
						<input type="text" id="name" class="form-control" required value="{{$res->first_name}} {{$res->last_name}}" disabled>
						</div>
						</div>
						...
						<div class="form-group">
						<input type="number"  name="mobile" id="phone" class="form-control" required  value="{{$res->mobile}}" disabled>
						</div>
						</div>						
						...
						<?php
						$courses = DB::table("course")->where('center_id',Auth::user()->id)->where('status',0)->get();
						foreach($courses as $course)
						{
						?>
						<option value="{{$course->id}}" <?php if($course->id == $courseRes->course_id){ echo "selected"; } ?>>{{$course->name}}</option>
						<?php } ?>
						</select>
						</div>
						</div>
						...
						<label>Batch</label>
						<div class="form-group">
						<select name="batch" class="form-control" required disabled>
						<option value="">Select Batch</option>
						<?php
						$btc = DB::table("course_batch")->where('course_id',$courseRes->course_id)->get();
						foreach($btc as $bt)
						{
						?>
						<option value="{{$bt->batch_name}}" <?php if($bt->batch_name == $courseRes->batch){ echo "selected"; } ?>>{{$bt->batch_name}}</option>
						<?php } ?>
						</select>
						</div>
						</div>		
						...
						<option value="0" <?php if($res->result == 0){ echo "selected"; } ?>>Pass</option>
						<option value="1" <?php if($res->result == 1){ echo "selected"; } ?>>Fail</option>
						<option value="2" <?php if($res->result == 2){ echo "selected"; } ?>>Not Declared</option>
						...
						<div class="form-group">
						<input type="text" name="grade" value="{{$res->grade}}" class="form-control" required>
						</div>
						</div>
						...
						{
						?>
						<a href="{{Asset('center/result/RemoveDocs/'.$doc->id.'/'.$doc->student_id)}}" onclick="return confirm('Are you sure want to remove?')" style="color:red;font-size:18px"><i class="fa fa-times"></i></a> <a href="{{Asset('upload/student/'.$doc->file)}}" target="_blank">{{$doc->file}}</a>
						<?php } ?>
						<br><br>
						<?php } ?>
